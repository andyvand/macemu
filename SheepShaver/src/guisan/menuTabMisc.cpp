...
namespace widgets
{
...
#ifdef ANDROIDSDL
  extern gcn::SDLTrueTypeFont* font2;
#endif
  ...
  class Jit68kActionListener : public gcn::ActionListener
  {
    public:
      void action(const gcn::ActionEvent& actionEvent)
      {
	    if (actionEvent.getSource() == checkBox_jit68k)
	       if (checkBox_jit68k->isSelected())
				menu_jit68k="true";
	       else
				menu_jit68k="false";
      }
  };
  Jit68kActionListener* jit68kActionListener;
  ...
  void menuTabMisc_Init()
  {
	...
	checkBox_jit68k = new gcn::CheckBox("Enable built-in 68k DR Emulator");
	checkBox_jit68k->setPosition(10,170);
	checkBox_jit68k->setId("jit68k");
	checkBox_jit68k->setBaseColor(baseCol);
	jit68kActionListener = new Jit68kActionListener();
	checkBox_jit68k->addActionListener(jit68kActionListener);
...
#endif
	if (menu_jit68k=="true")
	    checkBox_jit68k->setSelected(true);
	else
	    checkBox_jit68k->setSelected(false);
